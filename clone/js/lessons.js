...
var LessonManager = (function() {
    ...
    // WAVE1_SHARED_WITH_ME — open a lesson shared WITH this user. The recipient
    // is identity-authorised (target_type user/email), so we read the owner's
    // row by id and stage it as a `shared_` TEMPORARY, NON-SYNCABLE read copy
    // (see _isSyncableLesson / _isReadOnlyLessonSource — `shared_` is already a
    // first-class read-only/temporary lesson source here). Full co-edit write-back
    // for lessons is Wave 2 (needs contentSync, out of this wave's file scope).
    function _openSharedLesson(item) {
        ...
        ContentShare.fetchContentById(item.content_id).then(function (c) {
            ...
            var lesson = Object.assign({}, data, {
                id: 'shared_' + c.id,
                title: c.title || data.title || 'שיעור משותף',
                category: 'שותפו איתי',
                _temporaryLesson: true,
                _sharedRole: item.role || 'view',
                _tempCreatedAt: new Date().toISOString()
            });
            var all = loadLessons().filter(function (x) { return x.id !== lesson.id; });
            all.push(lesson);
            saveLessons(all);   // shared_ ids are non-syncable, so this stays local
            if (typeof startLessonViewer === 'function') startLessonViewer(lesson.id);
            else if (typeof openLessonEditor === 'function') openLessonEditor(lesson.id);
        });
    }
    ...
    function renderLessonsList() {
        ...
        // WAVE1_SHARED_WITH_ME — inject a "שותפו איתי" section listing lessons
        // shared WITH this user (hidden when none). Async; prepends to the list.
        if (typeof ContentShare !== 'undefined' && ContentShare.renderSharedInto) {
            ContentShare.registerOpener('lesson', _openSharedLesson);
            ContentShare.renderSharedInto(container, 'lesson', { label: 'שיעורים ששותפו איתי', openFn: _openSharedLesson });
        }
